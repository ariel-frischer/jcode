...
impl App {
    pub(super) fn command_help(&self, topic: &str) -> Option<String> {
        ...
        let help = match topic.as_str() {
            ...
            "handoff" => {
                "/handoff [prompt]\nCreate a clean child session and switch this client to it. With a prompt, that prompt is used as the fresh session's startup context. Without a prompt, Jcode generates a bounded handoff summary and carries forward open todo next steps; it starts automatically when auto_start is enabled, otherwise the summary is restored as an editable draft.\n\nUse /transfer when you want a summary-only transfer in a new window. Configure global defaults under [handoff] and per-profile overrides under [profiles.<name>.handoff]."
            }
            "plan" => {

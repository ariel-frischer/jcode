...
pub(in crate::tui::app) use followups::process_remote_followups;
#[cfg(test)]
use followups::{QUEUED_FOLLOWUP_STARVATION_TIMEOUT, format_stall_duration, stall_timeout};
use followups::{
    detect_and_cancel_stall, detect_starved_queued_followup, handle_debug_command,
    handle_mouse_event, queue_message_for_reconnect,
};

#[cfg(test)]
pub(super) use key_handling::reload_stale_remote_server_before_update;
use queue_recovery::{recover_local_interleave_to_queue, recover_undelivered_queued_continuation};
...
pub(super) async fn handle_tick(app: &mut App, remote: &mut RemoteConnection) -> bool {
    ...
    needs_redraw |= app.poll_file_mention_discovery();

...
fn send_server_event(tx: &mpsc::UnboundedSender<ServerEvent>, event: ServerEvent) -> bool {
    tx.send(event).is_ok()
}

async fn queue_control_interrupt(
    session_id: &str,
    content: String,
    urgent: bool,
    ...
    .await
    {
        crate::logging::debug(&format!(
            "TASK_CONTROL_INTERRUPT_UNDELIVERED session_id={session_id} urgent={urgent}"
        ));
    }
}
    ...
    let Some(route) = route else {
        if client_event_tx
            .send(ServerEvent::Error {
                id,
                message: format!(
                    "Task '{}' could not be assigned because no model route was resolved.",
                    selected_task_id
                ),
                retry_after_secs: None,
                provider_code: None,
...
mod task_control;
pub(super) use task_control::handle_comm_task_control;

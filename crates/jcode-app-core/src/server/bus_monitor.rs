//! Global bus event dispatch, including optional swarm completion wakes.
...
use super::file_activity_scope_label;
use super::{
    Agent, FileAccess, FileTouchService, NotificationType, Server, ServerEvent,
    SessionInterruptQueues, SharedContext, SwarmEvent, SwarmEventType, SwarmMember, VersionedPlan,
    latest_peer_touches, live_turn, queue_soft_interrupt_for_session, record_swarm_event,
    swarm_completion_wake,
};
    ...
    pub(super) async fn monitor_bus(
        ...
        loop {
            ...
            match receiver.recv().await {
                    ...
                    if !previous_touches.is_empty() {
                        ...
                        if let Some(member) = current_member {
                            for prev in &previous_touches {
                                ...
                                let intent_suffix =
                                    prev.intent.as_ref().map_or_else(String::new, |intent| {
                                ...
                                if member.event_tx.send(notification).is_err() {
                                    crate::logging::debug(
                                        "File-activity notification receiver disconnected",
                                    );
                                }
                        ...
                        for prev in &previous_touches {
                            if let Some(prev_member) = members.get(&prev.session_id) {
                                let scope = file_activity_scope_label(prev, &touch);
                                let intent_suffix =
                                    touch.intent.as_ref().map_or_else(String::new, |intent| {
                                        format!(" — intent: {}", intent)
                                    });
                                let alert_msg = format!(
                                ...
                                if prev_member.event_tx.send(notification).is_err() {
                                    crate::logging::debug(
                                        "Peer file-activity notification receiver disconnected",
                                    );
                                }
                ...
                Ok(BusEvent::SwarmMemberCompleted(event)) => {
                    swarm_completion_wake::dispatch_member_completion(
                        &event,
                        &sessions,
                        &soft_interrupt_queues,

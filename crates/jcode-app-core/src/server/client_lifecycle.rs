...
pub(super) async fn handle_client(
    ...
    let mut new_agent =
        crate::hooks::with_client_terminal_env(active_terminal_env.clone(), async {
            Agent::new_provisional_with_initial_working_dir_and_profile(
                Arc::clone(&provider),
                registry.clone(),
    ...
    loop {
        ...
        if matches!(&request, Request::ConfigureTools { .. } | Request::ListTools { .. } | Request::ToolResult { .. }) {
            ...
            let event = response.unwrap_or_else(|error| ServerEvent::Error { id, message: error.to_string(), retry_after_secs: None, provider_code: None });
            let _ = client_event_tx.send(event);
            continue;
        }
        ...
        if let Err(message) = resolve_target_subscribe_working_dir(
            ...
            let _ = client_event_tx.send(ServerEvent::Error {
                id: request.id(), message, retry_after_secs: None, provider_code: None,
            });
            continue;
        }
...
include!("client_lifecycle/message_streaming.rs");

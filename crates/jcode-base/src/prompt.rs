...
fn build_available_skills_section<'a, I>(available_skills: I) -> Option<String>
where
    I: IntoIterator<Item = &'a SkillInfo>,
{
    let available_skills: Vec<&SkillInfo> = available_skills.into_iter().collect();
...
fn build_system_prompt_full_with_overlay_and_capabilities(
    ...
    if let Some(skills_section) = build_available_skills_section(available_skills.iter().copied()) {
...
fn load_agents_md_files_from_dirs_with_replacement(
    ...
    let selected_duplicates_project = selected_path
        .is_some_and(|selected_path| same_canonical_path(&project_agents_md, selected_path));

...
Status: **Post-controller repair validation passed for the feature-owned paths; the incomplete representative benchmark matrix and feature-base-unchanged repository guardrail failures remain explicit delivery blockers**
...
| FR-005 | Execute equivalent isolated one-action benchmark lanes for adaptive, explicit 6-job, and explicit 8-job policies, then validate complete comparable sample sets. | `specs/015-tune-rust-feedback/evidence/job-counts.json`; benchmark receipts section below | N/A: controlled timing collection is an evidence boundary; `job-counts.json` records the incomplete trial and rejection rather than fabricated samples |
| FR-006 | Run the coordinated duplicate scenario and assert coordinator-authorized execution, global-gate ownership, and consistent requested/executed/follower/reused/coalesced counts. | `crates/jcode-app-core/src/tool/selfdev/tests.rs`; `specs/015-tune-rust-feedback/evidence/job-counts.json` | PASS (exit 0): 50 exact selfdev tests include coordinator coalescing and ownership regressions |
...
| FR-020 | Run focused checks, `scripts/check_guardrails.sh`, coordinated self-development build/reload, resolve the active executable, and smoke-test the newly built binary through a unique socket. | Delivery sections below; build/reload and isolated-socket receipts | FAIL (exit 1): focused checks, coordinated publish/reload, resolved live identity, runtime budgets, and isolated smoke passed; the unchanged repository guardrail gate failed on seven feature-base-unchanged checks |
...
- Failed all-target/all-feature compilation on feature-base-unchanged E2E fixtures: duplicate `active_skill` in `tests/e2e/test_support/mod.rs:393` (`E0062`) and missing `reported_cost_usd` in `tests/e2e/provider_behavior.rs:125` (`E0063`).
...
| Confirm live executable identity | `readlink -f "$HOME/.jcode/builds/shared-server/jcode"`; resolve `/proc/$(pgrep -f "$HOME/.jcode/builds/shared-server/jcode serve" | head -1)/exe`; `sha256sum` | PASS (exit 0) | Channel and live process both resolved to `$HOME/.jcode/builds/versions/bd0a995eb-dirty-02de59dfcb73/jcode`; version `jcode v0.79.637-dev (bd0a995eb, dirty)`; SHA-256 `51419de208451131fa58c6be256ec243c65c39c66309f3f1caf9e554853898d6`. |
...
|---|---|---|---|
| Candidate executable path | `realpath target/selfdev/jcode` | `$WORKTREE/target/selfdev/jcode` | PASS (exit 0) |
| Resolved executable path | `readlink -f target/selfdev/jcode` | `$WORKTREE/target/selfdev/jcode`; the candidate is a regular worktree executable, not the 70-byte shared-server symlink | PASS (exit 0) |
...
| Shared daemon identity after coordinated reload | `readlink -f "$HOME/.jcode/builds/shared-server/jcode"`; resolve the matching live `/proc/<pid>/exe`; `sha256sum` | `$HOME/.jcode/builds/versions/bd0a995eb-dirty-02de59dfcb73/jcode`; live process resolves to the same path; version `jcode v0.79.637-dev (bd0a995eb, dirty)`; SHA-256 `51419de208451131fa58c6be256ec243c65c39c66309f3f1caf9e554853898d6` | PASS (exit 0) |
...
| Check | Exact command or observation | Result | Evidence |
|---|---|---|---|
| Autospec schemas | `autospec artifact` for `spec.yaml`, `plan.yaml`, `tasks.yaml`, and `.autospec/constitution.yaml` | PASS (all exit 0) | Spec: 5 stories and 26 requirements; plan: 5 implementation phases; tasks: 35 tasks with T034 in progress during validation; constitution: 10 principles. No feature checklists or additional Autospec schema-governed artifacts exist. |
| Benchmark matrix and retained receipt schemas | `python3 scripts/bench_rust_feedback.py validate-matrix scripts/rust_feedback_matrix.json`; `validate-receipt` for each of the five retained JSON receipts | PASS (all exit 0) | `baseline.json`, `focused-scope.json`, `job-counts.json`, `nextest.json`, and `sccache.json` validated. Receipt validation now requires every aggregate reproducible from retained raw samples and rejects missing, extra, or mismatched aggregate fields. |
...
| Compatibility and precedence | Review `scripts/dev_cargo.sh`, `scripts/test_dev_cargo_scope.sh`, `scripts/test_dev_cargo_jobs.sh`, and the focused validation ledger | PASS | Scope inference is opt-in through non-empty `JCODE_DEV_CARGO_AFFECTED_PATHS`; absent input preserves the established Cargo arguments. Explicit package, target, feature, no-default-feature, and all-feature arguments win; broad, workspace, all-feature, and release-sensitive requests remain broad. Non-default `JCODE_DEV_FEATURE_PROFILE` values conservatively retain broad scope so root-package feature names are not applied to an inferred leaf package. Existing `JCODE_BUILD_JOBS` then `CARGO_BUILD_JOBS` precedence, adaptive fallback, Cargo test executor, cache defaults, and release/full-feature paths remain unchanged. |
| Queue, preflight, and concurrency ownership | Review `crates/jcode-app-core/src/tool/selfdev/build_queue.rs`, its exact tests, and the coordinated duplicate receipt | PASS | Test preflight activates only when `JCODE_RUST_VALIDATION_SCOPE_SCRIPT` is explicitly configured. The canonical resolver accepts an explicitly supplied bounded discovery snapshot/request through environment JSON. Preflight is limited to five seconds and 64 KiB, persists only allowlisted bounded fields, attributes follower metadata to the producer, and conservatively falls back on missing, timed-out, nonzero, malformed, stale, or non-proving evidence. Only a current proven-empty result returns before leader/follower claim. All executable requests still use the server-owned coordinator and the unchanged host-wide Cargo gate; no uncontrolled Cargo concurrency path was added. |
| Fresh behavioral closure | `bash scripts/test_dev_cargo_scope.sh`; `bash scripts/test_dev_cargo_jobs.sh`; `python3 -m unittest scripts.test_rust_validation_scope scripts.test_bench_rust_feedback`; `scripts/dev_cargo.sh test -p jcode-app-core --lib tool::selfdev::tests -- --nocapture`; `scripts/dev_cargo.sh check -p jcode-app-core` | PASS (all exit 0) | Both shell contract suites passed, including five non-default feature-profile cases; all 56 Python tests passed; all 51 exact Rust selfdev tests passed in the assigned worktree; and the relevant crate check passed. These checks cover complete aggregate enforcement, experiment-aware comparability, declared commands, descendant cleanup, accurate retry accounting, cross-package explicit-feature fallback, the canonical preflight entry point, bounded sanitized persistence, follower provenance, proven-empty rejection, conservative queueing, and one-producer duplicate coordination. |
| Post-controller repair and guardrail attribution | `JCODE_DEV_CARGO_SCRIPT="$PWD/scripts/dev_cargo.sh" scripts/check_guardrails.sh`; feature-base changed-path comparison; `cargo fmt --all --check` after the one feature-owned formatting repair | PASS for feature-owned repair checks; FAIL (exit 1) for the unchanged repository gate | The correctly worktree-bound run exposed one feature-owned rustfmt difference, which was fixed and the minimal format gate then passed. Seven gates remain failed. Their compile, Clippy, warning, size, panic, and swallowed-error paths are absent from the feature diff against `85f9f5e3`; the three warning-budget paths are `state_ui_input_helpers.rs` and `translate.rs`. No ratchet baseline was changed. |
...
| Final delivery boundary | Compare the completed review with the final acceptance summary and retained guardrail/benchmark evidence | PASS for T035 and repair review; FAIL for release readiness | The surgical and compatibility review found no additional blocker or unauthorized default change. It also does not waive the incomplete representative p50/p95 matrix or the seven feature-base-unchanged full-feature guardrail failures. The branch remains not releasable from this evidence set. |
...
| Full-feature guardrails passed | FAIL (exit 1) | The correctly worktree-bound post-repair run left seven failures. Exact failing compile, Clippy, warning, oversized-file/test, panic, and swallowed-error paths are unchanged from feature base `85f9f5e3`; focused success was not substituted and no ratchet baseline was changed. |
...
Final delivery decision: **FAIL / not releasable from this evidence set.** T035 and the bounded post-controller repair establish that the feature-owned code, tests, compatibility boundaries, and concurrency ownership pass focused validation, and no candidate default was authorized. That evidence does not override the incomplete representative benchmark comparison or the feature-base-unchanged failing full-feature guardrail gate.

...
| Benchmark matrix and retained receipt schemas | `python3 scripts/bench_rust_feedback.py validate-matrix scripts/rust_feedback_matrix.json`; `validate-receipt` for each of the five retained JSON receipts | PASS (all exit 0) | `baseline.json`, `focused-scope.json`, `job-counts.json`, `nextest.json`, and `sccache.json` validated. Receipt validation recomputed every scenario aggregate from retained raw samples through `_validate_aggregates`; mismatched reported percentiles would fail validation. |
...
| Fresh behavioral closure | `bash scripts/test_dev_cargo_scope.sh`; `bash scripts/test_dev_cargo_jobs.sh`; `python3 -m unittest scripts.test_rust_validation_scope scripts.test_bench_rust_feedback`; four exact `jcode-app-core` selfdev tests for proven-empty rejection, valid/uncertain fallback queueing, and duplicate attachment | PASS (all exit 0) | Both shell contract suites passed, all 48 Python tests passed, and all four exact Rust tests passed. The observed behavior directly confirms unchanged default/job precedence, conservative scope fallback, rejection before request persistence only for proven-empty evidence, normal queueing for valid or uncertain requests, and one-producer duplicate coordination with terminal-result propagation. |

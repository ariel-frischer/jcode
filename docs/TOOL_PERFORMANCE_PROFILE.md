...
| `jcode-5sx` | Reduce concurrent `agentgrep grep` aggregate CPU | Baseline attribution found one ripgrep subprocess followed by per-matched-file rereads and post-processing workers sized from host parallelism; existing targeted-grep evidence was 16.410 ms wall / 72.007 ms aggregate CPU. No paired randomized single/concurrent campaign or fork-level acceptance completed, so no safe general-purpose candidate was established | Closed investigation-only; no Jcode or fork production change retained |

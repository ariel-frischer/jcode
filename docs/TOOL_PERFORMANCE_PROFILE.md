...
| `jcode-5sx` | Reduce concurrent `agentgrep grep` aggregate CPU | Exact-revision baseline and candidate were paired over five randomized semantic cases plus 1/2/4/8-call concurrency attribution. Baseline aggregate child CPU medians were 10.61, 21.23, 47.43, and 102.40 ms; the worker-cap-4 candidate was 17.44, 32.20, 62.91, and 102.28 ms, with slower wall medians at every level. Output hashes matched literal, regex, paths-only, type, glob, hidden, no-ignore, and dense cases. | Terminal B; candidate rejected and fork change reverted. Baseline attribution confirms per-call ripgrep plus file reread/post-processing workers, but this cap is not a material general-purpose win without a wall regression |

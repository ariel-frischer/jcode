...
The integrated acceptance Bead `jcode-znm` is complete. The original redesign Beads remain open only for their broader campaign criteria: explicit cold-first timing, persistent-worker CPU attribution outside parent `RUSAGE_CHILDREN`, read-index eviction and read-side concurrency, and any security cases beyond the exercised `.gitignore` and sensitive-path checks. These results validate the integrated implementation and root-binary public smoke path without claiming those broader criteria. Rollback is the integration merge commit or the three scoped feature commit ranges for read, inventory, and hooks.

...
A preview is local-only and does not send or mutate file contents. User-clicked absolute and home-relative paths may point outside the repository. File metadata checks and reads run in background work so slow local mounts do not block TUI input, and a completed read is discarded if its originating message changed. Files larger than 512 KiB, binary or otherwise non-UTF-8 files, missing files, URLs, and mail links are rejected without opening a preview. Clearing the transcript also releases all retained preview content.
